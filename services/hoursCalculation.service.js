...
async function generateDailyAttendanceData({ departmentId, date, month }) {
    ...
    if (month) {
        ...
        end = moment.tz(date, TIMEZONE).endOf('day').add(1, 'day').format('YYYY-MM-DD HH:mm:ss'); // +1 day for Night Shift buffer
    } else {
    ...
    `, { replacements: { empCodes, start, end }, type: QueryTypes.SELECT, raw: true });
    ...
    `, { replacements: { companyMstId, departmentMstId, end, start }, type: db.Sequelize.QueryTypes.SELECT });
    ...
    if (month) {
        ...
        targetDates = Array.from({ length: totalDaysInMonth }, (_, i) =>
            moment.tz(month, 'YYYY-MM', TIMEZONE)
                .startOf('month')
                .add(i, 'days')
                .format('YYYY-MM-DD')
        );
    ...
    for (const emp of employees) {
        ...
        for (const dateStr of targetDates) {
            ...
            const validShifts = departmentShifts.filter(s => {
                ...
                const isBeforeTo = toDate ? currentDate.isSameOrBefore(toDate, 'day') : true;

                return isAfterFrom && isBeforeTo;
            });

            const todayPunches = empFlatPunches[dateStr] || [];
            const nextDayPunches = empFlatPunches[moment(dateStr).add(1, 'day').format('YYYY-MM-DD')] || [];
            const allAvailable = [...todayPunches, ...nextDayPunches].filter(p => !claimedPunchIds.has(p.id));
            ...
            const earliestStart = (assignedShift.IsPreShiftOT && assignedShift.PreShiftOTIn)
                ? moment.tz(`${dateStr} ${assignedShift.PreShiftOTIn}`, 'YYYY-MM-DD HH:mm:ss', TIMEZONE)
                : moment.tz(`${dateStr} ${assignedShift.ShiftIn}`, 'YYYY-MM-DD HH:mm:ss', TIMEZONE);
            const startBound = earliestStart.subtract(1, 'hours');

            // LATEST END (OT or Shift)
            let latestEnd = (assignedShift.IsPostShiftOT && assignedShift.PostShiftOTOut)
                ? moment.tz(`${dateStr} ${assignedShift.PostShiftOTOut}`, 'YYYY-MM-DD HH:mm:ss', TIMEZONE)
                : moment.tz(`${dateStr} ${assignedShift.ShiftOut}`, 'YYYY-MM-DD HH:mm:ss', TIMEZONE);
            ...
            const dayPunches = allAvailable.filter(p => {
                ...
                const isWithin = pTime.isSameOrAfter(startBound) && pTime.isSameOrBefore(endBound);
