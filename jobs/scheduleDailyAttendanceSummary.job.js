  ...
  cron.schedule(
    ...
    async () => {
      try {
          ...
          await Promise.all(
            batch.map(async (dept) => {
              try {
                ...
                console.log(
                  `Dept ${dept.DepartmentMstId} done. Records saved: ${result.savedCount} for ${yesterday}`,
                );
              } catch (deptErr) {
                console.error(
                  `[JOB ERROR] Dept ${dept.DepartmentMstId}:`,
                  deptErr.message,
                );
              }
            }),
        ...
        console.log(
          "Daily Attendance Summary Generation Completed Successfully",
        );
      } catch (err) {
