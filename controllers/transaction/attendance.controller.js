...
const {
  generateAndSaveDailyAttendanceSummary,
} = require("../../services/dailyAttendanceSummary.service");
const {
  calculateDepartmentSalary,
} = require("../../services/salaryCalculation.service");
const {
  getFaceDistance,
  isFaceMatch,
...
exports.addFacePunch = async (req, res) => {
  const { datetime, empCode } = req.body;
  const transaction = req.transaction;
  ...
  if (!empCode) {
    throw new Error("empCode is required.");
  }

  const matchedEmployee = await db.EmployeeMst.findOne({
    where: { EmpCode: empCode, Active: true },
