...
class AttendanceEngine {
  ...
  static calculateDayMetrics(metrics, dateStr, logs, shift, isNightShift) {
    ...
    let rawShiftOut = SalaryHelper.parseTime(
      dateStr,
      shift.ShiftOut,
      "ShiftOut",
    );
    ...
    const alignToShift = (timeStr, name) => {
      ...
      if (isNightShift) {
        const shiftStartMinutes =
          rawShiftIn.hours() * 60 + rawShiftIn.minutes();
    ...
    metrics.OriginalWorkingHours =
      SalaryHelper.minutesToHHMM(configWorkMinutes);
   ...
   /* ------------------- INSERT THE SEQUENCE VALIDATION HERE ------------------ */
    let expectedType = "IN";
    let hasSequenceError = false;

    for (let i = 0; i < sortedLogs.length; i++) {
      const currentType = sortedLogs[i].punchType?.toUpperCase();
    ...
    if (
      sortedLogs.length > 0 &&
      sortedLogs[sortedLogs.length - 1].punchType?.toUpperCase() !== "OUT"
    ) {
      hasSequenceError = true;
    }
    ...
    if (hasSequenceError) {
      ...
      metrics.WorkHours = "00:00";
      metrics.FinalTotalHours = "00:00";
      return metrics;
    }
    ...
    for (let i = 0; i < sortedLogs.length - 1; i++) {
      ...
      if (
        current.punchType?.toUpperCase() === "OUT" &&
        next.punchType?.toUpperCase() === "IN"
      ) {
        ...
        if (
          gapStart.isSameOrAfter(shiftStart) &&
          gapEnd.isSameOrBefore(shiftEnd)
        ) {
    ...
    const firstInLog = sortedLogs.find(
      (x) => x.punchType?.toUpperCase() === "IN",
    );
    const lastOutLog = [...sortedLogs]
      .reverse()
      .find((x) => x.punchType?.toUpperCase() === "OUT");
    ...
    metrics.FinalTotalHours = SalaryHelper.minutesToHHMM(
    ...
    metrics.EarlyOutMinutes = SalaryHelper.minutesToHHMM(
